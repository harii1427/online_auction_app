...
def auction_bidding():
    ...
    highest_bid = st.session_state.get("highest_bid", selected_item_details["base_price"])
    winning_participant = st.session_state.get("winning_participant", "")
    ...
    for participant in participants:
        if st.button(f"Bid by {participant}", key=participant):
            if bid_amount > highest_bid:
                ...
                st.session_state.highest_bid = highest_bid
                st.session_state.winning_participant = winning_participant
                st.success(f"Bid placed by {participant} successfully!")
            else:
                st.warning("Bid amount should be higher than the current highest bid.")

    # Sell item
    if st.button("Sold"):
        if winning_participant:
            st.success(f"Item sold to {winning_participant} for {highest_bid}!")
            ...
            st.warning("No bids received, cannot sell the item.")
